...
async def list_chat_messages(
    limit: Annotated[
        int, Field(ge=1, le=1000, description="Maximum number of messages to return")
    ] = 100,
    cursor: Annotated[
    ...
    async with get_client() as client:
        ...
        logger.info(
            f"Listing chat messages: limit={limit}, cursor={cursor}, include_deleted={include_deleted}"
        )
...
async def get_chat_message(
    ...
    include_deleted: Annotated[
        bool, Field(description="Allow retrieving deleted messages")
    ] = False,
) -> ChatMessage:
    """
...
async def create_chat_message(
    ...
    message_type: Annotated[
        Optional[str], Field(description="Message type (text, system, etc.)")
    ] = None,
    tags: Annotated[
...
async def update_chat_message(
    ...
    async with get_client() as client:
        ...
        response = await call_put(
            client, f"/v1/chat_messages/{uid}", json=payload, if_match=if_match
        )
        data = response.json()
